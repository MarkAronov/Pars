...
describe('GET /api/feed — chronological feed for the authed user', () => {
	it("includes the user's own posts and followed users' posts, not others'", async () => {
		...
		const viewer = await signUpAndLogin('exprFeedViewer');
		const followed = await signUpAndLogin('exprFeedFollowed');
		const stranger = await signUpAndLogin('exprFeedStranger');
		...
		const ownPost = await request(app)
			...
			.expect(201);
		const followedPost = await request(app)
			.post('/api/posts')
