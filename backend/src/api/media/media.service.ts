...
export class MediaService {
	...
	async uploadAvatar(uploaderId: string, file: Express.Multer.File): Promise<{ url: string }> {
		...
		await this.prisma.user.update({ where: { id: uploaderId }, data: { avatarUrl: url } });
	...
	async uploadBackground(uploaderId: string, file: Express.Multer.File): Promise<{ url: string }> {
		const mimeType = await this.validateImageMime(file.buffer);
		const ext = mimeType.split('/')[1];
		const filename = `${uploaderId}-${Date.now()}.${ext}`;
		const url = await this.saveFile(file.buffer, filename, 'backgroundImage');

		await this.prisma.user.update({ where: { id: uploaderId }, data: { backgroundUrl: url } });
		await this.prisma.media.create({
	...
	async deleteMedia(mediaId: string, requesterId: string): Promise<{ message: string }> {
